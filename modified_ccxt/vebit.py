...
class vebit (Exchange):

    def describe(self):
        return self.deep_extend(super(vebit, self).describe(), {
            'id': 'vebit',
            'name': 'vebit',
            'countries': 'US',
            'version': '0',
            ...
            'urls': {
                ...
                'api': 'https://vebit.com/info/api',
                'www': 'https://www.vebit.com',
                'doc': [
                    'None',
                    'None',
                ],
    ...
    def parse_ticker(self, ticker, symbol:str):
        ...
        try:
            ...
            result = {
                ...
                'open': float(symbol_info.__getitem__('Open')),
                'close': None,
                'first': None,
                'last': float(symbol_info.__getitem__('Last')),
                'change': None,
                'percentage': None,
                'average': None,
                'baseVolume': 'TRY',
            ...
            ask = result.__getitem__('ask')
            bid = result.__getitem__('bid')
            if ask > bid * 6:
                ask = bid * 1.002
            result.update({'ask':ask})
            return result
    ...
    def fetch_ticker(self, symbol, params={}):
        ...
        url = "https://www.vebitcoin.com/Ticker/" + str(first).upper()
        r = requests.get(url)
